...
_N_CONTINUOUS = 4        # velo, spin, plate_x, plate_z
...
CONTINUOUS_DIM: int = (
    _N_CONTINUOUS + _N_RESULT + _N_COUNT + _N_OUTS + _N_RUNNERS + _N_PITCH_NUMBER
)  # = 50
...
class V2InputLayer(nn.Module):
    ...
    def __init__(self, cfg: V2Config) -> None:
        ...
        # Projects the 50-dim state vector → d_model
        self.continuous_proj = nn.Linear(CONTINUOUS_DIM, d)
    ...
    def forward(
        ...
        # 1. Type embedding.
        te = self.type_emb(type_ids)      # (B, T, d)
        ...
        pe = self.pos_emb(pitch_number)       # (B, T, d)
